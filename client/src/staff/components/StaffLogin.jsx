...
const StaffLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const errorMsg =
        error.response?.data?.message || "Invalid staff credentials";
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 dark:bg-[#05070a] text-slate-900 dark:text-white font-sans selection:bg-indigo-500/30 p-4 relative overflow-hidden transition-colors duration-500">
      {/* Background Effects */}
      ...
      <div className="relative z-10 w-full max-w-md bg-white/70 dark:bg-[#161b22]/60 backdrop-blur-2xl p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-white/50 dark:border-white/10 animate-fade-in-up overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-transparent via-indigo-500 to-transparent opacity-80"></div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="relative w-full py-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white rounded-2xl font-bold text-lg shadow-xl shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-1 transition-all duration-300 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 overflow-hidden group/btn"
          >
        ...
        <div className="mt-8 text-center flex flex-col gap-2">
          <p className="text-xs text-slate-400 dark:text-slate-500 font-mono">
            BVC Digital Hub • Secure Faculty Gateway
          </p>
